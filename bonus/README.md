...
│       ├── deployment.yaml
│       ├── service.yaml
│       └── ingress.yaml    # reaches the app on :80 without a port-forward
└── scripts/
...
curl http://localhost/          # {"status":"ok", "message": "v2"}
```

`curl` goes through the Ingress, not a port-forward: a forward dies exactly when
Argo CD replaces the pod, which is the moment the evaluator is watching.
...
- The image is pinned to `gitlab/gitlab-ce:19.2.1-ce.0`, never `:latest`. A
  defense has to be reproducible: with `:latest` an upstream release between
  rehearsal and evaluation silently changes the thing being graded (19.3 is due
  ~2026-08-20). Re-check the tag the day before defending.
