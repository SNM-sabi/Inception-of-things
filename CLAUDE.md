...
- **`--node-ip` is the one flag that matters.** NIC 1 always belongs to Vagrant — that is how `vagrant ssh` reaches the box. Under VirtualBox it is NAT and every VM gets the identical `10.0.2.15`; under vagrant-libvirt (observed on slice 1.1) it is a DHCP lease on the `vagrant-libvirt` management network, e.g. `eth0 = 192.168.121.46`, and it carries the default route. Either way it is the address K3s would pick by default, and either way it is wrong for the cluster. Without `--node-ip`, nodes register with it; the agent still joins so `get nodes` reads Ready, and the breakage stays hidden until p2's traffic test. `--flannel-iface` is cheap insurance. **`--advertise-address` already defaults to `--node-ip`** (noise). **`--bind-address=192.168.56.110` is harmful** — the apiserver stops listening on loopback while the generated kubeconfig still targets `127.0.0.1:6443`, breaking your own kubectl.
...
- **Never hardcode the interface name — p8 says so in an info box.** *"Modern Linux distributions use predictable network interface names (e.g., enp0s8, enp0s9) instead of eth0/eth1… Adapt the commands according to your system's actual interface names."* Observed on slice 1.1: the official `debian/trixie64` box under libvirt names NIC 1 `eth0` (the box disables predictable naming), so NIC 2 will most likely be `eth1` — but bento boxes, other images, or a naming change would differ. Derive it from the IP at runtime instead.
- **ROLES reads `control-plane`, not `control-plane,master`.** The legacy `master` label was removed upstream and K3s v1.36 no longer applies it — verified live on slice 1.4 (`node-role.kubernetes.io/control-plane` is the only role label present). Older screenshots (including the subject's era) show both; ours showing one is correct, not a regression.
- **`--tls-san` is redundant here — with proof.** K3s puts `--node-ip` into the API serving cert automatically: `openssl s_client` against `192.168.56.110:6443` shows `IP Address:192.168.56.110` in the SANs with no `--tls-san` passed. Keep the flag out; cite the openssl output if asked.
...
- **Pre-seed the join token; do not copy the generated one.** Define it once in the Vagrantfile and pass it to *both* provisioners via `env:`. Copying `/var/lib/rancher/k3s/server/node-token` through `/vagrant` commits a live credential into the repo (observed on slice 1.1: our `/vagrant` is a live two-way **NFS** mount of the project directory, so a guest-written token lands next to the Vagrantfile instantly) and leaves a stale token that survives `vagrant destroy` (agent then fails with `token CA hash does not match`). Whether `/vagrant` is NFS (our setup), 9p, or one-way rsync is a provider/config detail — every variant is either a credential leak or a broken handoff. Defense answer: *"I inverted the dependency — instead of the agent reading a token the server generated, I told the server which token to use."*
...
- **app3 must be `spec.defaultBackend` — in its own RULE-LESS Ingress (observed on slice 2.5).** A host-less rule *appears* to work, but Traefik v3 orders routers by rule-string length, so it only loses to `app1.com` by accident; `defaultBackend` compiles to priority `math.MinInt32` and cannot be outranked. **However, Traefik v3.7 ignores `defaultBackend` when the same Ingress also carries rules** — verified live: the API object showed the default, the router never materialised, bare-IP curl stayed 404. The working shape is two objects: `apps` (the host rules) + `apps-default` (only the defaultBackend). The singleton constraint is on the *default declaration*, not the object count — a second Ingress declaring a default is silently dropped. `kubectl describe ingress` with no name still shows both on one screen for the p11 walkthrough. **Never add `traefik.ingress.kubernetes.io/router.priority`** to the default object — it would outrank the host rules.
...
- **Never start libvirt's `default` network INSIDE the outer VM.** The outer VM's own uplink is `192.168.122.x` (the laptop's libvirt NAT), and the nested default network uses the *same* `192.168.122.0/24` — starting it creates a duplicate connected route and kills the VM's internet. Debian ships it inactive/no-autostart (verified); leave it that way. vagrant-libvirt doesn't need it: it creates its own management network on `192.168.121.0/24`, which collides with nothing (laptop NAT `.122`, lab `.56`).
